    ...
    const submitHandler = async (e) => {
        ...
        if (!login) {
            if (enteredEmail.current.value.length > 0 && enteredpass.current.value.length > 0 && enteredConfirmPass.current.value.length > 0) {
                if (enteredpass.current.value === enteredConfirmPass.current.value) {
                    let responce = await fetch(
                        'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyCBXaXy5LXXBlenALnnoG8EeVmRr5Kk4e0',
                        {
                            method: 'POST',
                            body: JSON.stringify({
            ...
            if (enteredEmail.current.value.length > 0 && enteredpass.current.value.length > 0) {
                let responce = await fetch(
                    'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyCBXaXy5LXXBlenALnnoG8EeVmRr5Kk4e0',
                    {
                        method: 'POST',
                        body: JSON.stringify({
                            email: enteredEmail.current.value,
                ...
                if (responce.ok) {
                    ...
                    try {
                        let responce = await fetch(
                            'https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyCBXaXy5LXXBlenALnnoG8EeVmRr5Kk4e0',
                            {
                                method: 'POST',
                                body: JSON.stringify({
