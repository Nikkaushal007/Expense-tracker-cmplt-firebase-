    ...
    const handleDelete = async (expense) => {
        ...
        try {
            let responce = await fetch(
                `https://expense-data-61b66-default-rtdb.firebaseio.com/${user}/${expense.Name}.json`,
                {
                    method: 'DELETE',
                    headers: {
        ...
        if (enteredaAmount.current.value.length > 0 && enteredaDesc.current.value.length > 0 && enteredaCatagory.current.value.length > 0) {
            try {
                ...
                if (responce.ok) {
                    ...
                    try {
                        let responce = await fetch(
                            `https://expense-data-61b66-default-rtdb.firebaseio.com/${user}.json`,
                            {
                                method: 'GET',
                                headers: {
                                    'Content-Type': 'application/json',
    ...
    return (
        <Provider store={store}>
            <div className='container'>
                ...
                {CalculateTotalExpense(expenses) >= 10000 && <div className='text-center'><button className='btn btn-primary' onClick={handleActive}>Activate Premium</button></div>}
